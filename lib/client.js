window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Draw the retro sun: a glow, a gradient disc with cut bands, and a rim. */
		function drawSun(ctx, scene, palette) {
			...
			for (let i = 1; i < bands; i += 1) {
				const pr = i / bands;
				const y0 = cy - radius + pr * radius * 2;
				const thickness = .8 + Math.pow(pr, 2) * 9;
				ctx.fillRect(cx - radius - 2, y0, radius * 2 + 4, thickness);
			}
		...
		*/
		function drawFrame(ctx, scene, viewport, dark) {
			...
			const barY = scene.time * 130 % span - 110;
			const barTail = ctx.createLinearGradient(0, barY, 0, barY + 52);
			barTail.addColorStop(0, "rgba(0, 240, 255, 0.16)");
			barTail.addColorStop(1, "rgba(0, 240, 255, 0)");
			ctx.fillStyle = barTail;
			ctx.fillRect(0, barY + 2, width, 50);
